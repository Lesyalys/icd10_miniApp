...
import { Icon20Cancel, Icon24ChevronRight } from "@vkontakte/icons";
...
const App = () => {
  ...
  const searchInNode = (node, query, path = "") => {
    ...
    else if (searchType === "title" && matches(node.title, query))
      isMatch = true;
    else if (
      searchType === "all" &&
      (matches(node.code, query) || matches(node.title, query))
    )
      isMatch = true;
          ...
          <View activePanel="main">
            <Panel id="main">
              ...
              {searchQuery ? (
                <>
                  <Group>
                    {searchResults.length > 0 && (
                      <SimpleCell>Найдено: {searchResults.length}</SimpleCell>
                    )}
                    ...
                    {searchResults.slice(0, 20).map((result) => (
                      ...
                      >
                        <span
                          style={{
                            fontWeight: result.isRoot ? "bold" : "normal",
                          }}
                        >
                ...
                // Дерево МКБ-10
                <Group>{renderTree(icd10Data)}</Group>
              )}
            </Panel>
          </View>
        </SplitCol>
              ...
              <Group>
                <SimpleCell>
                  <Text weight="2" style={{ marginBottom: 4 }}>
                    Код: {selectedResult.code}
                  </Text>
                </SimpleCell>
                {/* <SimpleCell>
                  <Text weight="1" style={{ marginBottom: 4 }}>
                    Название: {selectedResult.title}
                  </Text>
                </SimpleCell> */}
                {selectedResult.children &&
                  selectedResult.children.length > 0 && (
                    <SimpleCell>
                      <Text weight="2">
                        Вложенных элементов: {selectedResult.children.length}
                      </Text>
                    </SimpleCell>
                  )}
              </Group>
                ...
                selectedResult.children.length > 0 && (
                  <Group header={<Header size="s">Дочерние элементы</Header>}>
                    ...
                    {selectedResult.children.length > 20 && (
                      <SimpleCell>
                        ... и еще {selectedResult.children.length - 20}{" "}
                        элементов
                      </SimpleCell>
                    )}
                  </Group>
